        ...
        arrNapor[j].input=(arrNapor[j].input).replace(',','.');
             if(arrNapor[j].input!=='' ){//если напор не пустая строка - продолжаем
                ...
                if(Number((arrTime[j]['gen'+i]).replace(',','.'))!==0){//если время не пустая строка или не равно 0 - продолжаем
                    ...
                    if(arrPower[j]['gen'+i]==='нет данных'){// если есть данные по генераторам - ошибка ввода
                        return `Введён напор за ${arrNapor[j].number}-й час суток, за этот час отсутствуют данные АИИСКУЭ (Сотрите напор и нажмите Enter)`
                    }else{  strRashod.push({number:i,data:'0'})}
                }            
            }else{//если напор пустая строка, проверка есть ли данные по генераторам
                if(arrPower[j]['gen'+i]!=='нет данных'){// если есть данные по генераторам - ошибка ввода
